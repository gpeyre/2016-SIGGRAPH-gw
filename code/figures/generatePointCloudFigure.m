...
clear
xyz = dlmread('../data/point_clouds/andyFacePointcloud.xyz');
xyz = xyz(1:10:end,:);
n0 = size(xyz,1);
D0 = distmat(xyz');
...
[X,T] = readOff('../data/point_clouds/face2.off');
n = size(X,1);
M = getMeshData(X,T);
D = distmat(X');
...
gg = dists;
generateSoftMapMeshes(M,M,'source_cloud.obj','garbage.obj',...
    [],[],points,gg',.02);